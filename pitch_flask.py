...
import random
import requests
import ollama  # Changed from Google Generative AI
...
OLLAMA_HOST = os.getenv('OLLAMA_HOST', 'http://localhost:11434')  # New config
BASE_RPM = 30
MAX_WORDS = 200
MAX_IMAGES = 3
...
class ContentGenerator:
    ...
    def generate_slide_content(self, section: str) -> tuple:
        ...
        try:
            ...
            content = self._process_text(response['response'])
...
def validate_env():
    missing = [k for k in ["PEXELS_API_KEY"] if not os.getenv(k)]  
    if missing:
